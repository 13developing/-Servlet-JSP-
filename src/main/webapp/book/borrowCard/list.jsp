                    ...
                    <div class="row">
                        <div class="col-sm-9 m-b-xs">
                            <c:if test="${sessionScope.loginUser.isAdmin}">
                                <a href="/book/borrowCardServlet?action=saveOrUpdatePage" class="btn btn-success "
                                   type="button"><i
                        ...
                        <div class="col-sm-3">
                            <form action="/book/borrowCardServlet" method="get" id="myForm">
                            ...
                            <c:forEach items="${requestScope.pageUtils.list}" var="entity">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <c:if test="${entity.state==2}"> 已过期</c:if>
                                        <c:if test="${entity.state==3}"> 已下架</c:if>
                                    </td>
                                    <td>
                                        <c:if test="${sessionScope.loginUser.isAdmin}">
                                            <c:if test="${entity.state!=3}">
                                                <a href="/book/borrowCardServlet?action=saveOrUpdatePage&id=${entity.id}"
                                                   class="btn btn-primary " type="button">
                                                    <i class="fa fa-edit"></i>&nbsp;更新</a>
                                            </c:if>
                                            <c:if test="${entity.state==3}">
                                                <a href="#" class="btn btn-primary " type="button">
                                                    <i class="fa fa-edit"></i>&nbsp;不可操作
                                                </a>
                                            </c:if>
                                            <c:if test="${entity.state==0}">
                                                <button class="btn btn-warning" onclick="removeData(${entity.id})"
                                                        type="button  "><i class="fa fa-remove"></i>&nbsp;下架
                                                </button>
                                            </c:if>
                                        </c:if>
                                    </td>
